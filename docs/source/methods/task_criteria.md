...
- **Process IDs** have the form `hed_<slug>` (e.g., `hed_response_inhibition`, `hed_visual_working_memory`). The `hed_` prefix is mandatory. The `hed_` prefix is this catalog's working prefix and is not a claim about HED-schema membership (see §2.5).
